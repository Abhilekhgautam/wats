...
json IRGenerator::Generate([[maybe_unused]] FunctionDefinitionAST &ast) {
  ...
  function_ir["name"] = ast.GetFunctionName();

  // Add arguments (Args are handled by the AST, assuming semantic analysis has
  // confirmed them)
  json args_array = json::array();

  auto opt_args = ast.GetFunctionArguments();
  if (opt_args.has_value()) {
    auto &fn_args = opt_args.value().get();

    // Iterate through all formal parameters defined in the signature
    for (const auto &id_ast : fn_args.GetIds()) {
      // For each argument, create an object defining its name and type
      args_array.push_back({
          {"name", id_ast->GetName()},
          {"type", id_ast->GetType()} // NOTE: Assuming i64 type for simplicity,
                                      // should use real type if available
      });
    }
  }
  function_ir["args"] = args_array;

  // --- 2. Generate the Function Body ---
  json instrs_array = json::array();

  // Loop through all statements (the function body)
  for (auto &stmt_ast : ast.GetFunctionBody()) {
    // Recursively generate IR for the statement
    json stmt_ir = Generate(*stmt_ast);

    // Collect the generated instructions. Statements can return either a single
    // instruction or an array (for loops, ifs, or complex assignments).
    if (stmt_ir.is_array()) {
      for (auto &instr : stmt_ir) {
        instrs_array.push_back(instr);
      }
    } else if (stmt_ir.is_object() && !stmt_ir.empty()) {
      // Only push single objects if they are valid instructions (i.e., not
      // empty placeholders)
      instrs_array.push_back(stmt_ir);
    }
  }

  function_ir["instrs"] = instrs_array;

  // NOTE: If your language requires an explicit 'ret' (return) instruction,
  // you must ensure one is added either here (if implicit) or generated by a
  // ReturnStatementAST visitor.
