...
class Token {
public:
    Token(TokenName tok_type, SourceLocation loc, std::string tok_value) :
        tok_type(tok_type), location(loc), tok_value(tok_value) {}

    Token(TokenName tok_type, SourceLocation loc, char tok_value) :
        tok_type(tok_type), location(loc), tok_value(std::string{tok_value}) {}

    friend std::ostream &operator<<(std::ostream &os, Token tok) {
        os << "line: " << tok.GetLine() << " col: " << tok.GetColumn() << " " << tok.tok_value << " " << tok.tok_type;
        return os;
    }
