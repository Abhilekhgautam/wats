...
public:
    ...
    void DidYouMean(std::string_view to_add, std::size_t line, std::size_t column, bool space_before = true) const;
...
private:
    ...
    std::unique_ptr<FunctionDefinitionAST> ParseFunctionWithRetType();
    std::unique_ptr<FunctionDefinitionAST> ParseFunctionWithoutRetType();
    ...
    std::unique_ptr<FunctionParameterAST> ParseFunctionParameters();

    std::unique_ptr<ExpressionAST> ParseFunctionParameter();

    // Utility for Parsing Variable Declaration
    std::unique_ptr<VariableDeclarationAST> ParseVariableDeclWithLet();
    std::unique_ptr<VariableDeclarationAST> ParseVariableDeclWithType();
    std::unique_ptr<VariableAssignmentAST> ParseVariableAssignment();
    std::unique_ptr<VariableDeclareAndAssignAST> ParseVariableInitWithLet();
    std::unique_ptr<VariableDeclareAndAssignAST> ParseVariableInitWithType();
    ...
    std::vector<std::unique_ptr<StatementAST>> ParseCurlyBraceAndBody();
    ...
    std::unique_ptr<FunctionCallExprAST> ParseFunctionCall();
